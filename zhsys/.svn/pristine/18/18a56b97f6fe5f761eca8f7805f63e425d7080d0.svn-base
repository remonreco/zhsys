...
function cleanError(){
    ...
    <form id="sf" action="convertLogin.html" method="post">
    <div class="bq-lend-cont">
        <h2>线上兑换登录系统</h2>
        <span>用户登录User Login</span>    
        <p><img src="images/username.png" ><input type="text" id="assetsAccount" name="assetsAccount" placeholder="请输入资产账号" data-placeholder="请输入资产账号" value="" onfocus="cleanError();"/></p>
        <p class="bq-password"><img src="images/password.png" ><input type="password" name="userPwd"  id="userPwd" placeholder="请输入密码" data-placeholder="请输入密码" value="" onfocus="cleanError();"/></p>
        <span style="text-align: center; padding: 5px">
               <a href="<%=request.getContextPath()%>/accountActive.jsp" target="_black" >账号激活</a>
		</span>
         <c:if test="${not empty sessionScope.convertError}">
			<span id="convertError" style="color:red;text-align: center; padding: 5px">
                                  	  登录失败：
				${sessionScope.convertError}
			</span>
			<% request.getSession().removeAttribute("convertError"); %>
		</c:if> 
